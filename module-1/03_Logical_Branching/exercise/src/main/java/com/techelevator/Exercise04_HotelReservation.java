...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if (numberOfNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE){
            return (Math.round(100*(numberOfNights * DISCOUNT_RATE)))/100.0;
        }
        return (Math.round(100*(numberOfNights * DAILY_RATE)))/100.0;
        //I know we haven't discussed the Math class yet, but it kept marking me as wrong because of the extra decimals.
        //I didn't google this. I just know basic Java. It takes a double, returns an int. I wanted a double to the hundredths place.
        //cue the redundant *100 /100.0
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        if (numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE){
            return numOfTotalNights*DISCOUNT_RATE + numOfWeekendNights * 10;
        }
